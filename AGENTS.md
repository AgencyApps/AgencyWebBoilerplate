...
When Codex has the Agency plugin, start ambiguous Agency tasks with capability discovery, resolve the `companyId`, and use plugin-backed control-plane tools instead of dashboard-only instructions or handwritten curl.
Before database-dependent or Agency-platform-dependent implementation, sync the Agency-managed env block into `.env.local`. Preserve unrelated local keys, use Agency's hosted `DATABASE_URL`, and surface pending database provisioning rather than inventing a local database fallback.
Ship repo-backed product changes to the developer origin and the Agency deploy remote. Keep origin pushes branch-aware, map deploy activation to Agency `main`, and surface a missing/ambiguous origin instead of pretending dual delivery happened.
...
- `agency/sdk/storage` for public asset upload tickets and app-owned public asset deletion through Agency.
- `agency/sdk/platform` for module manifest and platform event reads.
  ...
  Read `agency/sdk/INTEGRATIONS.md` before implementing Auth, Billing, Analytics, or public asset storage. Auth, Billing, and Analytics stay hidden in Agency until the app proves readiness:
...
- Analytics becomes live after trusted server code calls `initializeAgencyIntegration("analytics")` from `agency/sdk/platform`, or after the first accepted analytics event.
  Do not add direct Stripe, analytics vendor SDKs, or raw R2 credentials, and do not store platform secrets or live Agency runtime credentials in this repo.
  For public assets like uploaded images, logos, and documents intended for public viewing, use `agency/sdk/storage`; persist the returned keys or public URLs in app data instead of inventing object-storage plumbing.
